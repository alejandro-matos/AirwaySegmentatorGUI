...
class AirwaySegmenterGUI(tk.Frame):
    ...
    def convert_files(self):
        ...
        self.convert_to_stl(output_dir)
    ...
    def segment_airway(self, nifti_folder, output_dir):
        ...
        task_no = "Task001"
        nnUNet_IN = nifti_folder
        nnUNet_OUT = os.path.join(output_dir, "Segmentation")

        def script_execution():
            try:
                Path1 = self.resource_path("Airways/nnUNet_raw")
                Path2 = self.resource_path("Airways/nnUNet_results")
                Path3 = self.resource_path("Airways/nnUNet_preprocessed")

                os.environ["nnUNet_raw_data_base"] = Path1
                os.environ["RESULTS_FOLDER"] = Path2
                os.environ["nnUNet_preprocessed"] = Path3
    ...
    def create_widgets(self):
        ...
        rename_toggle.grid(row=roww+3, column=0, sticky="E", padx=(0,120), pady=(10, 5))

        # Name given to files
        nick_label = tk.Label(self, text="Name to be applied to files:", **label_style).grid(row=roww+3, columnspan=2,column=0, sticky="W", padx=(200, 0), pady=(10,5))
        self.nick_label_entry = tk.Entry(self, textvariable=self.data_nickname, width=9,font=styles.FONT_SIZE-3)
        self.nick_label_entry.grid(row=roww+3, column=1, sticky="E", padx=(0, 0), pady=(10, 5))
        ...
        self.start_number_entry = tk.Entry(self, textvariable=self.starting_number, width=5, font=styles.FONT_SIZE-3)
        self.start_number_entry.grid(row=roww+3, column=2, sticky="E", padx=(10, 80), pady=(10, 5))
        ...
        input_entry.grid(row=roww+4, columnspan=2,column=0, sticky="W", padx=(130, 0))
        ...
        input_button.grid(row=roww+4, column=2, sticky="W", padx=(10, 15))
